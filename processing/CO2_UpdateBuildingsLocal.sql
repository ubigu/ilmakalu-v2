DROP FUNCTION IF EXISTS public.CO2_UpdateBuildingsLocal;
...
muutos AS (
 ...
 FROM (SELECT t.xyind, t.rakv, t.energiam,
 		...
 		(CASE WHEN t.erpien_ala IS NOT NULL AND NOT(t.erpien_ala <= 0) THEN ARRAY(SELECT t.erpien_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'erpien' AND lammitysmuoto = t.energiam)
        )) END) as erpien,
 		(CASE WHEN t.rivita_ala IS NOT NULL AND NOT(t.rivita_ala <= 0) THEN ARRAY(SELECT t.rivita_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'rivita' AND lammitysmuoto = t.energiam)
        )) END) as rivita,
 		(CASE WHEN t.askert_ala IS NOT NULL AND NOT(t.askert_ala <= 0) THEN ARRAY(SELECT t.askert_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'askert' AND lammitysmuoto = t.energiam)
        )) END) as askert, 
 		(CASE WHEN t.liike_ala IS NOT NULL AND NOT(t.liike_ala <= 0) THEN ARRAY(SELECT t.liike_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'liike' AND lammitysmuoto = t.energiam)
        )) END) as liike,
 		...
 		(CASE WHEN t.tsto_ala IS NOT NULL AND NOT(t.tsto_ala <= 0) THEN ARRAY(SELECT t.tsto_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'tsto' AND lammitysmuoto = t.energiam)
        )) END) as tsto, 
 		(CASE WHEN t.liiken_ala IS NOT NULL AND NOT(t.liiken_ala <= 0) THEN ARRAY(SELECT t.liiken_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'liiken' AND lammitysmuoto = t.energiam)
        )) END) as liiken,
 		(CASE WHEN t.hoito_ala IS NOT NULL AND NOT(t.hoito_ala <= 0) THEN ARRAY(SELECT t.hoito_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'hoito' AND lammitysmuoto = t.energiam)
        )) END) as hoito,
 		(CASE WHEN t.kokoon_ala IS NOT NULL AND NOT(t.kokoon_ala <= 0) THEN ARRAY(SELECT t.kokoon_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'kokoon' AND lammitysmuoto = t.energiam)
        )) END) as kokoon,
 		(CASE WHEN t.opetus_ala IS NOT NULL AND NOT(t.opetus_ala <= 0) THEN ARRAY(SELECT t.opetus_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'opetus' AND lammitysmuoto = t.energiam)
        )) END) as opetus,
		(CASE WHEN t.teoll_ala IS NOT NULL AND NOT(t.teoll_ala <= 0) THEN ARRAY(SELECT t.teoll_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'teoll' AND lammitysmuoto = t.energiam)
		)) END) as teoll,
		(CASE WHEN t.varast_ala IS NOT NULL AND NOT(t.varast_ala <= 0) THEN ARRAY(SELECT t.varast_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'varast' AND lammitysmuoto = t.energiam)
		)) END) as varast,
		(CASE WHEN t.muut_ala IS NOT NULL AND NOT(t.muut_ala <= 0) THEN ARRAY(SELECT t.muut_ala * 
			UNNEST((SELECT ARRAY[kaukolampo, kevyt_oljy, kaasu, sahko, puu, maalampo] FROM energy."heat_source_change" WHERE scenario = kehitysskenaario AND buildingtype = 'muut' AND lammitysmuoto = t.energiam)
		)) END) as muut
